...
public class NodeComparatorWithSubstituents implements NodeComparator 
{
	...
	public int compare(GlycoNode o1, GlycoNode o2) {
		...
		try {	
			...
			if (m_visNodeType.isMonosaccharide(o1) &&
				...
				v.clear();			
				// Substituenten
				if ( !checkSubstituentsIdentity (o1,o2)){
					return 1;
				}				
				// Monosaccharide Name
	...
	private boolean checkSubstituentsIdentity(GlycoNode m1, GlycoNode m2) throws GlycoVisitorException {
		...
		for (GlycoEdge g:m1.getChildEdges()){
			if (m_visNodeType.isSubstituent(g.getChild())){				
				if (!hasSubstituent(m2,g)){
					return false;
				}				
			}			
		}	
