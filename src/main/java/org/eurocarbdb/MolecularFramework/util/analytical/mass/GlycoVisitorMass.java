...
//        this.m_iPerMePos = t_visPositions.getPerMePositions();
	...
	{
		SubstituentType t_objSubstType = a_objSubstituent.getSubstituentType();
		if ( t_objSubstType == SubstituentType.PHOSPHATE || t_objSubstType == SubstituentType.SULFATE || t_objSubstType == SubstituentType.N_SULFATE )
		{
			double t_dIncMass = 0;
			...
			{
				throw new GlycoMassException("Error with max. linkage count) of substituent " + a_objSubstituent.getSubstituentType().getName() + "." );
			}
