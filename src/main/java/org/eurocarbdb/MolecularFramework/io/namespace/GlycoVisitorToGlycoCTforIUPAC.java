...
public class GlycoVisitorToGlycoCTforIUPAC implements GlycoVisitor {
	...
	public GlycoVisitorToGlycoCTforIUPAC(MonosaccharideConversion a_objTranslator) {
		super();
	...
	public void visit(SugarUnitRepeat a_objRepeate)
			throws GlycoVisitorException {
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			GlycoVisitorToGlycoCTforIUPAC t_objVisitor = new GlycoVisitorToGlycoCTforIUPAC(
	...
	public void visit(UnvalidatedGlycoNode a_objResidue)
		...
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			...
			try {
				...
				monosaccharideDetail.put("b", 0);
				monosaccharideDetail.put("a", 0);

				String replaceName = a_objResidue.getName();
				
				if (replaceName.matches("[A-Z].{2,5}" + "b")) {
					// Delete "L-" from residue name and flag the exist of
					// L-type monosaccharide.
					replaceName = replaceName.replaceAll("([A-Z].{2,5})b", "$1");
					a_objResidue.setName(replaceName);
					monosaccharideDetail.put("b", 1);
				}
				if (replaceName.matches("[A-Z].{2,5}" + "a")) {
					// Delete "L-" from residue name and flag the exist of
					// L-type monosaccharide.
					replaceName = replaceName.replaceAll("([A-Z].{2,5})a", "$1");
					a_objResidue.setName(replaceName);
					monosaccharideDetail.put("a", 1);
				}
				
				t_objData = this.m_objConverter.convertMonosaccharide(
						a_objResidue.getName(), GlycanNamescheme.CARBBANK,
						GlycanNamescheme.GLYCOCT);
				if(t_objData.getBasetype().getBaseType(0).getName().matches(".*glc")){
					try {
						t_objData.getBasetype().setRing(1, 5);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						// TODO 自動生成された catch ブロック
						e.printStackTrace();
					}
				}
				if(t_objData.getBasetype().getBaseType(0).getName().matches(".*man")){
					try {
						t_objData.getBasetype().setRing(1, 5);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						// TODO 自動生成された catch ブロック
				...
				if(t_objData.getBasetype().getBaseType(0).getName().matches(".*gal")){
					try {
						t_objData.getBasetype().setRing(1, 5);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						// TODO 自動生成された catch ブロック
				...
				if(t_objData.getBasetype().getBaseType(0).getName().matches(".*xyl")){
					try {
						t_objData.getBasetype().setRing(1, 5);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						// TODO 自動生成された catch ブロック
					...
					if(t_objData.getBasetype().getBaseType(0).getName().matches(".*ara")){
						try {
							t_objData.getBasetype().setRing(1, 5);
						} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
							// TODO 自動生成された catch ブロック
				...
				/*
				 * if(monosaccharideDetail.get("a-") == 1){ try {
				 * t_objData.getBasetype().setAnomer(EcdbAnomer.Alpha); } catch
				 * (org.eurocarbdb.resourcesdb.glycoconjugate_derived.
				 * GlycoconjugateException e) { // TODO 自動生成された catch ブロック
				 * e.printStackTrace(); }
				 * 
				 * }
				 */
				if (monosaccharideDetail.get("b") == 1) {
					EcdbAnomer replaceAnomer = EcdbAnomer.Beta;
					try {
						t_objData.getBasetype().setAnomer(replaceAnomer);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						// TODO 自動生成された catch ブロック
						e.printStackTrace();
					};
				}
				if (monosaccharideDetail.get("a") == 1) {
					EcdbAnomer replaceAnomer = EcdbAnomer.Alpha;
					try {
						t_objData.getBasetype().setAnomer(replaceAnomer);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						// TODO 自動生成された catch ブロック
