...
/**
 * iterieren ueber alle residues ==> aufbauen einer hashmap ==> etablieren der
 * Linkage
 * 
 * @author Logan
 * 
 */
public class GlycoVisitorToGlycoCTforKCF implements GlycoVisitor {
	...
	public GlycoVisitorToGlycoCTforKCF(MonosaccharideConversion a_objTranslator) {
		super();
		this.m_objConverter = a_objTranslator;
	}

	public GlycoVisitorToGlycoCTforKCF(MonosaccharideConversion a_objTranslator,
			GlycanNamescheme a_strSchema) {
		super();
		this.m_objConverter = a_objTranslator;
	...
	public void setMonosaccharideConversion(
			MonosaccharideConversion a_objConverter) {
	 ...
	 */
	public void visit(Monosaccharide a_objMonosaccharid)
			throws GlycoVisitorException {
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			try {
				...
				// copy linkage
				this.copyParentLinkage(a_objMonosaccharid.getParentEdge(),
						t_objMS);
			} catch (GlycoconjugateException e) {
				throw new GlycoVisitorException(
						"Could not create monosaccharide : " + e.getMessage(),
						e);
			}
		}
	}
	 ...
	 */
	public void visit(SugarUnitRepeat a_objRepeate)
			throws GlycoVisitorException {
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			GlycoVisitorToGlycoCTforKCF t_objVisitor = new GlycoVisitorToGlycoCTforKCF(
					this.m_objConverter, this.m_strSchema);
	...
	public void visit(UnvalidatedGlycoNode a_objResidue)
			throws GlycoVisitorException {
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			// do normalisation
			try {
				MonosaccharideExchangeObject t_objData = null;

				// This code was added by yuki for replace linkage infomation of
				// sialic acid.
				Map<String, Integer> monosaccharideDetail = new HashMap<String, Integer>();
				monosaccharideDetail.put("a-", 0);
				monosaccharideDetail.put("L-", 0);
				monosaccharideDetail.put("-ol", 0);
				monosaccharideDetail.put("-onic", 0);

				String replaceName = a_objResidue.getName();

				if (replaceName.matches(".*[A-Z][a-z][a-z]" + "p" + ".*")) {
					replaceName = replaceName.replaceAll("p", "");
					// Delete "p" from residue name.
					a_objResidue.setName(replaceName);
				}

				if (replaceName.matches(".*" + "D-" + ".*")) {
					// Delete "D-" from residue name.
					replaceName = replaceName.replaceAll("D-", "");
					a_objResidue.setName(replaceName);
				}


				if (replaceName.matches(".*" + "L-" + ".*")) {
					// Delete "L-" from residue name and flag the exist of
					// L-type monosaccharide.
					replaceName = replaceName.replaceAll("L-", "");
					a_objResidue.setName(replaceName);
					monosaccharideDetail.put("L-", 1);
				}

				if (replaceName.matches(".*" + "-ol")) {
					// Delete "-ol" from residue name and flag the exist of aldi
					// substituent.
					replaceName = replaceName.replaceAll("-ol", "");
					a_objResidue.setName(replaceName);
					monosaccharideDetail.put("-ol", 1);
				}

				t_objData = this.m_objConverter.convertMonosaccharide(
						a_objResidue.getName(), this.m_strSchema,
						GlycanNamescheme.GLYCOCT);
				
				if (monosaccharideDetail.get("L-") == 1) {
					String replaceBaseType = t_objData.getBasetype()
							.getBaseType(0).getName();
					EcdbBaseType beRemovedBaseType = t_objData.getBasetype()
							.getBaseType(0);
					t_objData.getBasetype().removeBaseType(beRemovedBaseType);
					replaceBaseType = replaceBaseType.replaceAll("^d|^x", "l");
					try {
						t_objData.getBasetype().addBaseType(
								EcdbBaseType.forName(replaceBaseType));
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						e.printStackTrace();
					}
				}

				if (monosaccharideDetail.get("-ol") == 1) {
					try {
						t_objData.getBasetype().setRing(-1, -1);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						e.printStackTrace();
					}
					EcdbModificationType additionalAldiType = EcdbModificationType.ALDI;
					EcdbModification additionalAldi = null;
					try {
						additionalAldi = new EcdbModification(
								additionalAldiType, 1);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						// TODO 自動生成された catch ブロック
						e.printStackTrace();
					}
					try {
						t_objData.getBasetype().addModification(additionalAldi);
					} catch (org.eurocarbdb.resourcesdb.glycoconjugate_derived.GlycoconjugateException e) {
						// TODO 自動生成された catch ブロック
						e.printStackTrace();
					}
				...
				// ]]]This code was added by yuki for replace linkage infomation
				// of sialic acid
				...
				try {
					...
					throw new GlycoVisitorException(
							"Could not create normalized monosaccharide : "
									+ e.getMessage(), e);
				}
				ArrayList<Integer> t_aPositions;
				// attache Sustituents
				for (Iterator<SubstituentExchangeObject> t_iterSubst = t_objData
						.getSubstituents().iterator(); t_iterSubst.hasNext();) {
					SubstituentExchangeObject t_objSubstMSdb = t_iterSubst
							.next();
					Substituent t_objSubst = new Substituent(
							SubstituentType.forName(t_objSubstMSdb.getName()));
					// create Edge
					...
					t_objLinkage
							.setParentLinkageType(LinkageType
									.forName(t_objSubstMSdb.getLinkagetype1()
											.getType()));
					t_objLinkage
							.setChildLinkageType(LinkageType.NONMONOSACCHARID);
					// fill with linkage positions (parent)
					// 0 = unknown attach position
					t_aPositions = t_objSubstMSdb.getPosition1();
					for (Iterator<Integer> t_iterPosition = t_aPositions
							.iterator(); t_iterPosition.hasNext();) {
						Integer t_iPosition = t_iterPosition.next();
						if (t_iPosition == 0 || t_iPosition == -1) {
							if (t_aPositions.size() != 1) {
								throw new GlycoVisitorException(
										"Linkage for substituent "
												+ t_objSubst
														.getSubstituentType()
														.getName()
												+ " can only be unknown or a destinct linkage, not both.");
							}
							t_objLinkage
									.addParentLinkage(Linkage.UNKNOWN_POSITION);
						} else {
					...
					for (Iterator<Integer> t_iterPosition = t_aPositions
						...
						if (t_iPosition == 0 || t_iPosition == -1) {
							...
							t_objLinkage
									.addChildLinkage(Linkage.UNKNOWN_POSITION);
						} else {
					...
					if (t_aPositions.size() > 0) {
						...
						t_objLinkage.setParentLinkageType(LinkageType
								.forName(t_objSubstMSdb.getLinkagetype2()
										.getType()));
						t_objLinkage
								.setChildLinkageType(LinkageType.NONMONOSACCHARID);
						// fill with linkage positions (parent)
				...
				if (t_objPEdge != null) {
					...
					if (t_objPEdge != null) {
						GlycoNode t_objParent = this.m_hashResidues
								.get(a_objResidue.getParentNode());
						if (t_objParent == null) {
							throw new GlycoVisitorException(
									"Critical error: missing parent residue for subtituent : "
											+ t_objMS.getGlycoCTName());
						}
						// This code was added by yuki for replace linkage
						// infomation of sialic acid
						ArrayList<BaseType> sialicAcidBasetype = new ArrayList<BaseType>();
						sialicAcidBasetype.add(BaseType.DGRO);
						sialicAcidBasetype.add(BaseType.DGAL);
						sialicAcidBasetype.add(BaseType.DGLC);
						if (t_objMS.getBaseType().equals(sialicAcidBasetype)) {
							Linkage p_linkage = t_objPEdge
									.getGlycosidicLinkages().get(0);
							p_linkage.getChildLinkages().remove(0);
							p_linkage.addChildLinkage(2);
						}
						// This code was added by yuki for replace linkage
						// infomation of sialic acid
						this.m_objUnit
								.addEdge(t_objParent, t_objMS, t_objPEdge);
					}
				}
			} catch (ResourcesDbException e) {
				// not a monosaccharide
				try {
					SubstituentExchangeObject t_objSubstExchange = new SubstituentExchangeObject(
							this.m_strSchema);
					
					t_objSubstExchange.setName(a_objResidue.getName());
					t_objSubstExchange = this.m_objConverter
							.convertSubstituent(t_objSubstExchange,
									this.m_strSchema, GlycanNamescheme.GLYCOCT);
					
					Substituent t_objSubst = new Substituent(
							SubstituentType.forName(t_objSubstExchange
									.getName()));
					...
					try {
						...
						throw new GlycoVisitorException(
								"Could not create Substituten : "
										+ t_objSubst.getSubstituentType()
												.getName(), e2);
					}
					// linkage type
					int t_iCounter = 1;
					if (this.m_objInternalOriginalEdge != null) {
						...
						if (this.m_objInternalOriginalEdge.getChild() == a_objResidue) {
							for (Iterator<Linkage> t_iterLinkages = this.m_objInternal
									.getGlycosidicLinkages().iterator(); t_iterLinkages
									.hasNext();) {
								Linkage t_objLinkage = t_iterLinkages.next();
								t_objLinkage
										.setChildLinkageType(LinkageType.NONMONOSACCHARID);
								t_objLinkage.setParentLinkageType(this
										.getLinkageType(t_iCounter++,
												t_objSubstExchange));
							}
						}
					}
					...
					if (t_objPEdge != null) {
						...
						if (t_objPEdge != null) {
							// there is a parent
							for (Iterator<Linkage> t_iterLinkages = t_objPEdge
									.getGlycosidicLinkages().iterator(); t_iterLinkages
									.hasNext();) {
								Linkage t_objLinkage = t_iterLinkages.next();
								
								//this souce code was added by yuki to chage sulfate and phosphate Linkage infomrmation[[[
								if (t_objSubstExchange.getName().matches("sulfate") || t_objSubstExchange.getName().matches("phosphate")) {
										ArrayList<Integer> tmpSubstPosition = new ArrayList<Integer>();
										Integer intOfOne = Integer.valueOf(1);
										tmpSubstPosition.add(intOfOne);
										t_objLinkage.setChildLinkages(tmpSubstPosition);
								}
								//]]]this souce code was added by yuki to chage sulfate and phosphate Linkage infomrmation
								t_objLinkage
										.setChildLinkageType(LinkageType.NONMONOSACCHARID);
							...
							this.m_objUnit.addEdge(t_objParent, t_objSubst,
									t_objPEdge);
						}
					}
					// add edge to sugar
					for (Iterator<GlycoEdge> t_iterEdge = a_objResidue
							.getChildEdges().iterator(); t_iterEdge.hasNext();) {
						...
						for (Iterator<Linkage> t_iterLinkages = t_objPEdge
							...
							Linkage t_objLinkage = t_iterLinkages.next();
							t_objLinkage
									.setParentLinkageType(LinkageType.NONMONOSACCHARID);
							t_objLinkage.setChildLinkageType(this
									.getLinkageType(t_iCounter++,
											t_objSubstExchange));
						}
					}
					...
					try {
						...
						if (this.m_bStrict) {
							...
							String t_strName;
							try {
								t_strName = this.m_objConverter.convertAglycon(
										a_objResidue.getName(),
										this.m_strSchema,
										GlycanNamescheme.GLYCOCT);
								NonMonosaccharide t_objNonMS = new NonMonosaccharide(
										t_strName);
								this.m_hashResidues.put(a_objResidue,
										t_objNonMS);
								this.m_objUnit.addNode(t_objNonMS);
								// copy parent
								GlycoEdge t_objPEdge = a_objResidue
										.getParentEdge();
								if (t_objPEdge != null) {
									t_objPEdge = a_objResidue.getParentEdge()
											.copy();
									if (t_objPEdge != null) {
										GlycoNode t_objParent = this.m_hashResidues
												.get(a_objResidue
										...
										this.m_objUnit.addEdge(t_objParent,
												t_objNonMS, t_objPEdge);
									}
								}
							} catch (ResourcesDbException e2) {
								// not a valid aglyca
								throw new GlycoVisitorException(
										"Unknown residue (aglyca?): "
												+ a_objResidue.getName());
							}
						} else {
							...
							if (t_objPEdge != null) {
								...
								if (t_objPEdge != null) {
									...
									if (t_objParent == null) {
										throw new GlycoVisitorException(
												"Critical error: missing parent residue for historical data : "
														+ t_objNonMS.getName());
									}
									this.m_objUnit.addEdge(t_objParent,
											t_objNonMS, t_objPEdge);
	 ...
	 */
	public GlycoTraverser getTraverser(GlycoVisitor a_objVisitor)
			throws GlycoVisitorException {
	...
	public void start(Sugar a_objSugar) throws GlycoVisitorException {
		...
		if (this.m_objConverter != null) {
			...
			// speczial trees
			try {
				for (Iterator<UnderdeterminedSubTree> t_iterSpezialTrees = a_objSugar
						.getUndeterminedSubTrees().iterator(); t_iterSpezialTrees
						.hasNext();) {
					GlycoVisitorToGlycoCTforKCF t_objVisitor = new GlycoVisitorToGlycoCTforKCF(
							this.m_objConverter, this.m_strSchema);
					t_objVisitor.setUseStrict(this.m_bStrict);
					t_objVisitor
							.setMonosaccharideConversion(this.m_objConverter);
					UnderdeterminedSubTree t_objSubtreeOriginal = t_iterSpezialTrees
							.next();
					UnderdeterminedSubTree t_objSubtree = t_objVisitor
							.start(t_objSubtreeOriginal);
					this.m_objNewSugar.addUndeterminedSubTree(t_objSubtree);
					// copy properties
					if (t_objSubtreeOriginal.getConnection() != null) {
						t_objSubtree.setConnection(t_objSubtreeOriginal
								.getConnection().copy());
					}
					...
					// copy parents
					for (Iterator<GlycoNode> t_iterParents = t_objSubtreeOriginal
							.getParents().iterator(); t_iterParents.hasNext();) {
						GlycoNode t_objParent = this.m_hashResidues
								.get(t_iterParents.next());
						if (t_objParent == null) {
							throw new GlycoVisitorException(
									"Crictial error by adding null as a parent of a UnderdeterminedSubTree.");
						}
						this.m_objNewSugar.addUndeterminedSubTreeParent(
								t_objSubtree, t_objParent);
					}
				}
				// correkt alternative attache positions
				for (Iterator<AlternativeMapping> t_iterAlternative = this.m_aAlternative
						.iterator(); t_iterAlternative.hasNext();) {
					AlternativeMapping t_objAlternative = t_iterAlternative
							.next();
					...
					HashMap<GlycoNode, GlycoNode> t_hMapOld = t_objAlternative
							.getGraphOriginal().getLeadOutNodeToNode();
					HashMap<GlycoNode, GlycoNode> t_hIntern = t_objAlternative
							.getMapping();
					for (Iterator<GlycoNode> t_iterPositions = t_hMapOld
							.keySet().iterator(); t_iterPositions.hasNext();) {
						...
						if (t_objOuterNew == null) {
							throw new GlycoconjugateException(
									"Error child attache position of alternative graph was not translated.");
						}
						t_objInnerOld = t_hMapOld.get(t_objOuterOld);
						t_objInnerNew = t_hIntern.get(t_objInnerOld);
						if (t_objInnerNew == null) {
							throw new GlycoconjugateException(
									"Error child inner attache position of alternative graph was not translated.");
						}
						t_hMapNew.put(t_objOuterNew, t_objInnerNew);
					}
					t_objAlternative.getCopy().setLeadOutNodeToNode(t_hMapNew,
							t_objAlternative.getGraphCopy());
				}
			} catch (GlycoconjugateException e) {
				throw new GlycoVisitorException(e.getMessage(), e);
			}
	 ...
	 * @throws GlycoVisitorException
	 */
	protected SugarUnitRepeat start(SugarUnitRepeat a_objSugar)
			throws GlycoVisitorException {
		this.clear();
		if (this.m_objConverter != null) {
			// create new Sugar object
			...
			try {
				...
				for (Iterator<UnderdeterminedSubTree> t_iterSpezialTrees = a_objSugar
					...
					for (Iterator<GlycoNode> t_iterParents = t_objSubtreeOriginal
						...
						this.m_objRepeat.addUndeterminedSubTreeParent(
								t_objSubtree, t_objParent);
					}
				}
				// correkt alternative attache positions
	 ...
	 * @throws GlycoVisitorException
	 */
	protected UnderdeterminedSubTree start(UnderdeterminedSubTree a_objSubtree)
			throws GlycoVisitorException {
		this.clear();
		if (this.m_objConverter != null) {
			// create new Sugar object
	...
	protected void copyParentLinkage(GlycoEdge a_objOrigin, GlycoNode a_objNode)
			throws GlycoVisitorException {
		if (a_objOrigin != null) {
			GlycoNode t_objParent = this.m_hashResidues.get(a_objOrigin
					.getParent());
			if (t_objParent == null) {
				throw new GlycoVisitorException(
						"Critical error while translating: could not found a parent residue.");
			}
	 ...
	 */
	public void visit(NonMonosaccharide a_objResidue)
			throws GlycoVisitorException {
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			try {
				// copy residue
				...
				throw new GlycoVisitorException(
						"Could not create nonmonosaccharide : "
								+ e.getMessage(), e);
			}
		}
	}
	 ...
	 */
	public void visit(Substituent a_objSubstituent)
			throws GlycoVisitorException {
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			try {
				// copy residue
				...
				this.m_objUnit.addNode(t_objMS);
				// copy linkage
				this.copyParentLinkage(a_objSubstituent.getParentEdge(),
						t_objMS);
			} catch (GlycoconjugateException e) {
				throw new GlycoVisitorException(
						"Could not create substitutent : " + e.getMessage(), e);
			}
		}
	}
	...
	public void visit(SugarUnitCyclic a_objCyclic) throws GlycoVisitorException {
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			try {
				GlycoNode t_objStart = this.m_hashResidues.get(a_objCyclic
						.getCyclicStart());
				GlycoEdge t_objEdge = a_objCyclic.getParentEdge();
				if (t_objEdge == null) {
					throw new GlycoVisitorException(
							"Critical error in cyclic unit, no parent edge.");
				}
				GlycoNode t_objParent = this.m_hashResidues.get(a_objCyclic
						.getParentNode());
				if (t_objStart == null || t_objParent == null) {
					throw new GlycoVisitorException(
							"Critical error in cyclic unit.");
				}
				// copy linkage
				...
				throw new GlycoVisitorException(
						"Could not create cyclic residue : " + e.getMessage(),
						e);
			}
		}
	}
	...
	public void visit(SugarUnitAlternative a_objAlternative)
		...
		if (this.m_objTraverser.getState() == GlycoTraverser.ENTER) {
			...
			for (Iterator<GlycoGraphAlternative> t_iterGraphs = a_objAlternative
					.getAlternatives().iterator(); t_iterGraphs.hasNext();) {
				GlycoGraphAlternative t_objGraph = t_iterGraphs.next();
				GlycoVisitorToGlycoCTforKCF t_objVisitor = new GlycoVisitorToGlycoCTforKCF(
						this.m_objConverter, this.m_strSchema);
				t_objVisitor.setUseStrict(this.m_bStrict);
				GlycoGraphAlternative t_objGraphCopy = t_objVisitor.start(
						t_objGraph, t_objCopy);
				this.m_aAlternative.add(new AlternativeMapping(
						a_objAlternative, t_objGraph, t_objCopy,
						t_objGraphCopy, t_objVisitor.getResidueMapping()));
			}
			...
			try {
				...
				throw new GlycoVisitorException(
						"Could not create alternative : ", e);
			}
		}
	}

	protected Monosaccharide createMS(EcdbMonosaccharide a_objMsDB)
			throws GlycoconjugateException {
		// anomer & superclass
		Monosaccharide t_objMS = new Monosaccharide(Anomer.forSymbol(a_objMsDB
				.getAnomer().getSymbol().charAt(0)),
				Superclass.forCAtomCount(a_objMsDB.getSuperclass()
						.getNumberOfC()));
		// ring
		...
		// basetype
		for (Iterator<EcdbBaseType> t_iterBasetype = a_objMsDB
				.getBaseTypeList().iterator(); t_iterBasetype.hasNext();) {
			t_objMS.addBaseType(BaseType.forName(t_iterBasetype.next()
					.getName()));
		}
		// modification
		for (Iterator<EcdbModification> t_iterModification = a_objMsDB
				.getModificationList().iterator(); t_iterModification.hasNext();) {
			t_objMS.addModification(this.createModification(t_iterModification
					.next()));
		}
		return t_objMS;
	}

	protected Modification createModification(EcdbModification a_objModification)
			throws GlycoconjugateException {
		if (a_objModification.hasPositionTwo()) {
			return new Modification(a_objModification.getName(),
					a_objModification.getPositionOne(),
					a_objModification.getPositionTwo());
		}
		return new Modification(a_objModification.getName(),
				a_objModification.getPositionOne());
	}

	protected LinkageType getLinkageType(int a_iCounter,
			SubstituentExchangeObject a_objSubstExchange)
			throws GlycoconjugateException {
		if (a_iCounter == 1) {
			// position 1
			org.eurocarbdb.resourcesdb.glycoconjugate_derived.LinkageType t_objType = a_objSubstExchange
					.getLinkagetype1();
			if (t_objType != null) {
		...
		throw new GlycoconjugateException(
				String.format(
						"SubstituentExchangeObject does not contain information for linkage %d at substituten %s.",
						a_iCounter, a_objSubstExchange.getName()));
	}
	 ...
	 * @throws GlycoVisitorException
	 */
	protected GlycoGraphAlternative start(
			GlycoGraphAlternative a_objAlternative,
			SugarUnitAlternative a_objAlternativeUnit)
			throws GlycoVisitorException {
		this.clear();
		if (this.m_objConverter != null) {
			try {
				...
				if (t_objNode != null) {
					...
					if (t_objNode == null) {
						throw new GlycoVisitorException(
								"Error translating alternative parent attach node.");
					}
					a_objAlternativeUnit.setLeadInNode(t_objNode,
							this.m_objAlternative);
				}
			} catch (GlycoconjugateException e) {
				throw new GlycoVisitorException(e.getMessage(), e);
			}
