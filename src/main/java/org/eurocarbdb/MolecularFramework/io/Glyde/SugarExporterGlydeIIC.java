...
{
	private ArrayList<Reference> m_aReferences = new ArrayList<Reference>();
	...
	public void setReferences(ArrayList<Reference> a_aReferences)
	{
		this.m_aReferences = a_aReferences;
	}
	
	public void setGlycanId(String a_strID)
	{
		this.m_strGlycanID = a_strID;
	}
...
//	    }	    
	...
	public void start(Sugar a_objSugar) throws GlycoVisitorException 
	{
		...
		this.addReferences(this.m_objRootElement);
        try
        {
            this.export(a_objSugar);
        } 
	...
	private void addReferences(Element a_objRootElement) 
	{
	    if ( this.m_aReferences.size() > 0 )
	    {
	        Element t_elementModelRef = new Element("model_ref");
	        for (Reference t_reference : this.m_aReferences) 
	        {
	            Element t_elementResource = new Element("resource");
	            t_elementResource.setAttribute("name",t_reference.getReferenceType());
	            t_elementResource.setAttribute("id",t_reference.getReference());
	            t_elementModelRef.addContent(t_elementResource);
	        }
	        a_objRootElement.addContent(t_elementModelRef);
	    }
	}
	...
	private void export(Sugar a_objSugar) throws GlycoVisitorException, GlycoconjugateException
	{
		...
		for (Iterator<GlycoEdge> t_iterEdges = this.m_aEdges.iterator(); t_iterEdges.hasNext();) 
		{
			...
			for (Iterator<Linkage> t_iterLinkage = t_objEdge.getGlycosidicLinkages().iterator(); t_iterLinkage.hasNext();) 
			{
                ...
                {
                	t_objLinkageElement.setAttribute("to_replaces",t_objAtomNames.m_strToReplace);
                }
                if ( t_objAtomNames.m_strFromReplace != null )
                {
                	t_objLinkageElement.setAttribute("from_replaces",t_objAtomNames.m_strFromReplace);
                }
    ...
    private String getChildReplaceString(LinkageType a_objType, Linkage a_objLinkage,GlycoNode a_objNode, int a_iLinkNumber) throws GlycoVisitorException
    {
        ...
        {
            throw new GlycoVisitorException("Linkage type " + a_objType.getType() + " is not allowed.");
        }
	...
	private Atomnames getAtomnames(GlycoNode a_objParent, GlycoNode a_objChild, Linkage a_objLinkage,int a_iLinknumber) throws GlycoVisitorException 
	{
		...
		t_objResult.m_strFromReplace = this.getParentReplaceString(a_objLinkage.getParentLinkageType(),a_objLinkage,a_objParent,a_iLinknumber);
		t_objResult.m_strFrom = this.getChildString(a_objLinkage.getChildLinkageType(),a_objLinkage,a_objChild,a_iLinknumber);
		t_objResult.m_strToReplace = this.getChildReplaceString(a_objLinkage.getChildLinkageType(),a_objLinkage,a_objChild,a_iLinknumber);		
		return t_objResult;
	}
	...
	private String getSubstLink(Substituent a_objSubstituent, boolean a_bChildResidue,int a_iLinkNumber) throws GlycoVisitorException 
	{
		if (a_bChildResidue)
		{
			...
			if ( a_objSubstituent.getSubstituentType() == SubstituentType.R_CARBOXYETHYL )
			{
			    return "C1";
			}
