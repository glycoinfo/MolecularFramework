 ...
 *
 */
public class GlycoVisitorFromGlycoCTAnomer implements GlycoVisitor
{
     ...
     */
    public GlycoVisitorFromGlycoCTAnomer(MonosaccharideConversion a_objTranslator) 
    {
        super();
        this.m_objConverter = a_objTranslator;
    }

    public GlycoVisitorFromGlycoCTAnomer(MonosaccharideConversion a_objTranslator, GlycanNamescheme a_strSchema) 
    {
        super();
        this.m_objConverter = a_objTranslator;
    ...
    public void visit(SugarUnitRepeat a_objRepeate) throws GlycoVisitorException 
    {
        if ( this.m_objTraverser.getState() == GlycoTraverser.ENTER )
        {
            GlycoVisitorFromGlycoCTAnomer t_objVisitor = new GlycoVisitorFromGlycoCTAnomer( this.m_objConverter , this.m_strSchema );
...
//            }
    ...
    public void visit(Monosaccharide a_objMonosaccharid) throws GlycoVisitorException 
    {
        if ( this.m_objTraverser.getState() == GlycoTraverser.ENTER )
        {
            ...
            try
            {
                ...
                // fill the hashmap with the information about the anomer
                ExportAnomer t_anomer = new ExportAnomer();
                t_anomer.setAnomer(a_objMonosaccharid.getAnomer());
                t_anomer.setPosition(this.findAnomerPosition(a_objMonosaccharid));
                this.m_hashAnomer.put(t_objNonMS, t_anomer);
            }
            catch (ResourcesDbException e)
            {
    ...
    private Integer findAnomerPosition(Monosaccharide a_objMonosaccharid)
    {
        if ( a_objMonosaccharid.getAnomer() == Anomer.OpenChain )
        {
            return null;
        }
        if ( a_objMonosaccharid.getRingStart() != Monosaccharide.UNKNOWN_RING )
        {
            return a_objMonosaccharid.getRingStart();
        }
        Integer t_position = 50;
        for (Modification t_modi : a_objMonosaccharid.getModification())
        {
            if ( t_modi.getModificationType() == ModificationType.KETO )
            {
                if ( t_modi.getPositionOne() < t_position )
                {
                    t_position = t_modi.getPositionOne();
                }
            }
        }
        if ( t_position.equals(50))
        {
            t_position = 1;
        }
        return t_position;
